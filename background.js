...
function mapWindows(windows, groups){
	windows.forEach(function(window, no){
		//Find matched group for window
		var len = window.tabs.length;
		groups.every(function(group){
			var match = false;
			if(len == group.tabs.length){
				match = group.tabs.every(function(tab, i){
					return window.tabs[i].url == tab.url;
				});
			}
			if(match){
				group.id = window.id;
				//Find corresponding tab
				group.tabs.forEach(function(tab, no){
					tab.id = window.tabs[no].id;
				});
			}
			else{
				delete group.id;
...
chrome.storage.local.get({windows : null}, function(storage){
	windows = [];
	tabs = [];
	for(key in storage){
		...
		if(matches){
			...
			case "window-tab" :
				windows[matches[2]].tabs = storage[key];
			}
		}
	}
	if(storage.windows === undefined){
		chrome.windows.getAll({populate : true}, function(windows){
			var groups = windows.filter(function(window){
				return window.type == "normal";
			});
			chrome.windows.getAll({populate : true}, function(windows){
				mapWindows(windows, groups);
			});
			groups.forEach(function(group){
				windowManager.createWindow(group);
